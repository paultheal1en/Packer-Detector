 ...
 * @return The concatenated string on error return NULL
 */
static char* appendPath(const char* s1, const char* s2, const char* s3)
{
    ...
    *p = '\0';
    if (s1 && (strcpy_s(p, n, s1) != 0)) return NULL;
    if (s2 && (strncat_s(p, n, s2, strnlen_s(s2, MAX_PATH)) != 0)) return NULL;
    if (s3 && (strncat_s(p, n, s3, strnlen_s(s3, MAX_PATH)) != 0)) return NULL;
    return p;
}
